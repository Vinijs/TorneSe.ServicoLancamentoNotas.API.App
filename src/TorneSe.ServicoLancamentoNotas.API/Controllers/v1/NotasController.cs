...
using TorneSe.ServicoLancamentoNotas.App.Controllers.Base;

namespace TorneSe.ServicoLancamentoNotas.App.Controllers.v1;
...
public class NotasController : MainController
{
    ...
    public async Task<IActionResult> Lancar([FromBody] LancarNotaInput input, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Efetuado lançamento de nota do aluno {alunoId} da para a atividade {atividadeId}. {@response}",
    ...
    public async Task<IActionResult> Atualizar([FromBody] AtualizarNotaInput input, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Efetuado atualização de nota do aluno {alunoId} da para a atividade {atividadeId}. {@response}",
            input.AlunoId, input.AtividadeId, response);
    ...
    public async Task<IActionResult> Cancelar([FromBody] CancelarNotaInput input, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Efetuado cancelamento de nota do aluno {alunoId} da para a atividade {atividadeId}. {@response}",
            input.AlunoId, input.AtividadeId, response);
