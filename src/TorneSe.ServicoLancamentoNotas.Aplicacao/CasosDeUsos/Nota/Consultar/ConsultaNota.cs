...
public class ConsultaNota : IConsultaNota
{
    ...
    public async Task<Resultado<ListaNotaOutput>> Handle(ListaNotaInput request, CancellationToken cancellationToken)
    {
        try
        {
            var buscaOutput = await _notaRepository.Buscar(new BuscarInput(request.Pagina, request.PorPagina, request.AlunoId,
                                                           request.AtividadeId, request.OrdenarPor, request.Ordenacao),
                                                           cancellationToken);

            ListaNotaOutput retorno = new(buscaOutput.Pagina, buscaOutput.PorPagina, buscaOutput.Total,
                 buscaOutput.Items.Select(nota => MapeadorAplicacao.NotaEmNotaOutputModel(nota)).ToList().AsReadOnly());
