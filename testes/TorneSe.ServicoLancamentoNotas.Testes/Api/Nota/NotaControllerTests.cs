...
using TorneSe.ServicoLancamentoNotas.App.Controllers.v1;
using Xunit;
...
public class NotaControllerTests
{
    ...
    public async Task Lancar_QuandoResultadoSucesso_DeveRetornarOk()
    {
        ...
        _mediatorHandlerMock.Setup(x =>
                            x.EnviarRequest<Resultado<NotaOutputModel>, LancarNotaInput>(It.IsAny<LancarNotaInput>(),
                                                    It.IsAny<CancellationToken>()))
            .ReturnsAsync(retorno);

        var resultado = await _sut.Lancar(It.IsAny<LancarNotaInput>(), It.IsAny<CancellationToken>())
            as ObjectResult;
    ...
    public async Task Atualizar_QuandoResultadoSucesso_DeveRetornarOk()
    {
        ...
        _mediatorHandlerMock.Setup(x =>
                            x.EnviarRequest<Resultado<NotaOutputModel>, AtualizarNotaInput>(It.IsAny<AtualizarNotaInput>(),
                                                    It.IsAny<CancellationToken>()))
            .ReturnsAsync(retorno);

        var resultado = await _sut.Atualizar(It.IsAny<AtualizarNotaInput>(), It.IsAny<CancellationToken>())
            as ObjectResult;
        resultado.Should().NotBeNull();
        resultado!.StatusCode.Should().Be(StatusCodes.Status200OK);
    ...
    public async Task Cancelar_QuandoResultadoSucesso_DeveRetornarOk()
    {
        ...
        _mediatorHandlerMock.Setup(x =>
                            x.EnviarRequest<Resultado<NotaOutputModel>, CancelarNotaInput>(It.IsAny<CancelarNotaInput>(),
                                                    It.IsAny<CancellationToken>()))
            .ReturnsAsync(retorno);

        var resultado = await _sut.Cancelar(It.IsAny<CancelarNotaInput>(), It.IsAny<CancellationToken>())
            as ObjectResult;
        resultado.Should().NotBeNull();
        resultado!.StatusCode.Should().Be(StatusCodes.Status200OK);
    ...
    [Fact(DisplayName = nameof(Buscar_QuandoNaoDevolvidaNotas_DeveRetornarOkComListagemVazia))]
    [Trait("Api", "Nota - Controller")]
    public async Task Buscar_QuandoNaoDevolvidaNotas_DeveRetornarOkComListagemVazia()
    {
        var retorno = _fixture.RetornaResultadoComLista();

        _mediatorHandlerMock.Setup(x =>
                            x.EnviarRequest<Resultado<ListaNotaOutput>, ListaNotaInput>(It.IsAny<ListaNotaInput>(),
                                                    It.IsAny<CancellationToken>()))
            .ReturnsAsync(retorno);

        var resultado = await _sut.Buscar(It.IsAny<ListaNotaInput>(), It.IsAny<CancellationToken>())
            as ObjectResult;
        resultado.Should().NotBeNull();
        resultado!.StatusCode.Should().Be(StatusCodes.Status200OK);
    ...
    [Fact(DisplayName = nameof(Buscar_QuandoDevolvidaNotas_DeveRetornarOkComListagem))]
    [Trait("Api", "Nota - Controller")]
    public async Task Buscar_QuandoDevolvidaNotas_DeveRetornarOkComListagem()
    {
        var retorno = _fixture.RetornaResultadoComLista(10);
