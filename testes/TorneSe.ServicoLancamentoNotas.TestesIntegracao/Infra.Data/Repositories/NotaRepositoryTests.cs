...
public class NotaRepositoryTests
{
    ...
    public async Task Buscar_QuandoInformadaOrdenacao_DeveRetornarLista(OrdenacaoBusca ordenacao, string ordenarPor)
    {
        //Arrange
        var input = _fixture.RetornaBuscaInputApenasComPaginacao(ordenacao: ordenacao, ordenarPor: ordenarPor);
        var notas = _fixture.RetornarNotas();
        await _context.AddRangeAsync(notas);
        await _context.SaveChangesAsync();
