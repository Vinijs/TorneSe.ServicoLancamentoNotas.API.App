...
public class ConsultarNotaTests
{
    ...
    public async Task Handle_QuandoBuscaRetornaValores_DeveRetornarResultadoComSucessoEValores() 
    {
        ...
        var buscaInput = _fixture.RetornaListaBuscaInput();
    ...
    [Theory(DisplayName = nameof(Handle_QuandoBuscaOrdenada_DeveRetornarResultadoComSucessoEValores))]
    [Trait("Aplicacao", "Integracao/ConsultaNota - Casos de Uso")]
    ...
    public async Task Handle_QuandoBuscaOrdenada_DeveRetornarResultadoComSucessoEValores(OrdenacaoBusca ordenacao, string ordenarPor)
    {
        var notas = _fixture.RetornaNotasValidas();
        await _context.AddRangeAsync(notas);
        await _context.SaveChangesAsync();

        var buscaInput = _fixture.RetornaBuscaInputApenasComPaginacao(porPagina: 20, ordenacao: ordenacao, ordenarPor: ordenarPor);
